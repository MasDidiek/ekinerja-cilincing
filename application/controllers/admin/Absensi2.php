...
class Absensi extends CI_Controller
{
    ...
    function rekap_absensi()
    {
        ...
        $data["data_rekap"] = $this->Presensi_model->getDataRekapAbsensi(
            $periode,
            "mst_pegawai.nama",
            $jns_pegawai,
            $id_puskesmas,
    ...
    function view_absensi_pegawai($pin, $bulan, $tahun, $jns_pegawai = "")
    {
        ...
        $pegawai = $this->Presensi_model->getDetailPegawaiByPin(
            $pin,
            $jns_pegawai   );
        ...
        $data["cuti_pegawai"] = $this->acm->get_cuti_bulanan_absensi(
            $id_pegawai,
            $bulan,
            $tahun   );

        $data["absensi"] = $this->Presensi_model->get_absensi_pegawai(
            $pin,
            $bulan,
            $tahun   );
        $data["dataRekap"] = $this->Presensi_model->getRekapAbsensiPegawai(
            $id_pegawai,
            $periode   );
        $data["DinasLuar"] = $this->Presensi_model->getDataPengajuanDLPerbulan(
            $id_pegawai,
            $periode   );

        $data["IzinSakit"] = $this->Presensi_model->getDataIzinSakit(
            $id_pegawai,
            "ALL",
            $periode   );
    ...
    function update_absensi_cuti($pin, $id_cuti, $bulan, $tahun)
    {
        ...
        redirect(
            "admin/absensi/view_absensi_pegawai/" .
                $pin .
                "/" .
                $bulan .
                "/" .
    ...
    {
        $data["absensi_raw"] = $this->Sinkron_model->getDataAbsenMesin(
            $ip_address,
            $pin   );
        $data["bulan"] = $bulan;
        $data["tahun"] = $tahun;
        $data["detailPegawai"] = $this->Presensi_model->getDetailPegawaiByPin(
            $pin,
            $jns_pegawai   );
    ...
    function insertAbsenPengajuanDL($id, $pin, $bulan, $tahun)
    {
        ...
        $this->Presensi_model->saveAbsensiDL(
            $pin,
            $tanggal,
            $jns_dl,
            $keterangan   );
    ...
    {
        $qry = $this->db->get_where("pengajuan_izin_sakit", ["id" => $id]);
        $is = $qry->row();
        ...
        $this->Presensi_model->saveAbsensiIzinSakit(
            $pin,
            $tanggal,
            $jenis_absen,
            $status_detail,
    ...
    public function update_shift_pegawai($pin, $bulan, $tahun)
    {
        ...
        $data = $this->Presensi_model->getAbsensiPegawaiPerbulan(
            $pin,
            $periode   );
    ...
    function update_shift(
        ...
        if ($jns_shift == "non_shift") {
            ...
            $dataShift = $this->Shift_model->getShiftPerbulanById(
                $id_pegawai,
                $periode   );
            ...
            foreach ($dataShift as $shift_kerja) {
                ...
                $cekAbsen = $this->Presensi_model->cekAbsenExist(
                    $tanggal,
                    $pin,
                    "id");
    ...
    public function sinkron_absensi(
        ...
        // =========================
        $absensi_raw = $this->Sinkron_model->getDataAbsenMesin(
            $ip_address,
            $pin   );
        ...
        foreach ($absensi_raw as $row) {
            ...
            $jam_absen = $this->tentukan_jam_absen(
                $pin,
                $tanggal,
                $jam_masuk_shift,
                $jam_pulang_shift   );
    ...
    private function tentukan_jam_absen(
        $pin,
        $tanggal,
        $jam_masuk_shift,
        $jam_pulang_shift) {
    ...
    public function update_rekap_absensi($id_pegawai, $pin, $bulan, $tahun)
    {
        ...
        $kehadiran = $this->Presensi_model->getAbsensiPegawaiPerbulan(
            $pin,
            $periode   );
        ...
        foreach ($kehadiran as $row) {
            ...
            if ($izin) {
                ...
                if ($izin->jns_izin == 2) {
                    $this->update_status($row->id, 'IZIN', 'IZIN AWAL');
                }
    ...
    private function rekapDataAbsensi($id_pegawai, $pin, $bulan, $tahun)
    {
        ...
        foreach ($data as $row) {
            ...
            if ($row->status == "IZIN") {
                if ($row->status_detail == "IZIN FULL") {
                    $numIzin++;
                } else {
                    $izin_half++;
                }
    ...
    private function hitung_keterlambatan($id_pegawai, $pin, $bulan, $tahun)
    {
        ...
        foreach ($data as $row) {
            ...
            // ========================
            if (
                in_array($row->status_detail, ["DLP", "IZIN_FULL"]) ||
                $row->status == "SAKIT"
            ) {
                $this->update_menit($row->id, 0, 0);
                continue;
            ...
            $detailShift = $this->Presensi_model->detailShiftByKode(
                $row->shift,
                "row"   );
            ...
            if (
                $row->jam_masuk &&
                !in_array($row->status_detail, ["DLA", "IZIN_AWAL"])
            ) {
            ...
            if (
                $row->jam_pulang &&
                !in_array($row->status_detail, ["DLAK", "IZIN_AKHIR"])
            ) {
                ...
                if (!in_array($row->status_detail, ["DLAK", "IZIN_AKHIR"])) {
    ...
    //     $absen =  $this->Presensi_model->get_absensi_pegawai($pin, $bulan, $tahun);
    //     $shiftList = $this->db
    //         ->where('publish', 1) // kalau hanya yang aktif
    //         ->get('mst_shift_kerja')
    //         ->result();
    ...
    //         if (!isset($shiftMap[$row->shift])) {
    //             continue;
    //         }

    //         $tanggal            = $row->tanggal;

    //         $jam_masuk_standar  = strtotime($shiftMap[$row->shift]['jam_masuk']);
    //         $jam_pulang_standar = strtotime($shiftMap[$row->shift]['jam_pulang']);
    ...
    //             $jam_masuk_real = strtotime($row->jam_masuk);

    //             if ($jam_masuk_real > $jam_masuk_standar) {
    //                 $telat = floor(($jam_masuk_real - $jam_masuk_standar) / 60);
    //             } else {
    //                 $telat = 0;
    //             }

    //             $update['telat_menit'] = $telat;
    //         } else {
    //             $update['telat_menit'] = 0;
    //         }
    ...
    //             $jam_pulang_real = strtotime($row->jam_pulang);

    //             if ($jam_pulang_real < $jam_pulang_standar) {
    //                 $pulang_awal = floor(($jam_pulang_standar - $jam_pulang_real) / 60);
    //             } else {
    //                 $pulang_awal = 0;
    //             }

    //             $update['p_awal_menit'] = $pulang_awal;
    //         } else {
    //             $update['p_awal_menit'] = 0;
    //         }
    ...
    //             ==================================
    //             */
    //         $status_sekarang = strtoupper($row->status);
    //         $shift_sekarang  = strtoupper($row->shift);

    //         // daftar status yang tidak boleh diubah
    //         $status_protected = ['IZIN', 'DINAS', 'CUTI', 'SAKIT'];

    //         // 1?? Jika shift OFF ? selalu OFF
    //         if ($shift_sekarang == 'OFF') {
    ...
    //             // 2?? Jika status termasuk protected ? jangan ubah
    //             if (in_array($status_sekarang, $status_protected)) {

    //                 // tidak mengubah status
    //                 // tapi tetap nolkan telat & pulang awal
    //                 $update['telat_menit'] = 0;
    //                 $update['p_awal_menit'] = 0;
    //             }
    //             // 3?? Jika jam kosong semua ? ALPHA
    ...
    //                 $update['p_awal_menit'] = 0;
    //             }
    //             // 4?? Jika ada salah satu jam ? HADIR
    //             else {
    ...
    //         if (isset($izinMap[$tanggal])) {

    //             $izin = $izinMap[$tanggal];
    ...
    //                 =====================================
    //                 */
    //             if ($izin->jenis_absen == 'IZIN') {

    //                 if ($izin->jns_izin == 1) { // FULL

    //                     $update['status'] = 'IZIN';
    //                     $update['status_detail'] = 'IZIN FULL';
    //                     $update['telat_menit'] = 0;
    //                     $update['p_awal_menit'] = 0;
    //                 } elseif ($izin->jns_izin == 2) { // IZIN AWAL

    //                     $update['status'] = 'IZIN';
    //                     $update['status_detail'] = 'IZIN AWAL';
    //                     $update['telat_menit'] = 0;
    //                     // pulang awal tetap dihitung (jangan diubah)

    //                 } elseif ($izin->jns_izin == 3) { // IZIN AKHIR

    //                     $update['status'] = 'IZIN';
    //                     $update['status_detail'] = 'IZIN AKHIR';
    //                     $update['p_awal_menit'] = 0;
    //                     // telat tetap dihitung
    ...
    //                 JIKA SAKIT
    //                 =====================================
    //                 */ elseif ($izin->jenis_absen == 'SAKIT') {

    //                 $update['status'] = 'SAKIT';

    //                 if ($izin->jns_sakit == 1) {
    //                     $update['status_detail'] = 'SAKIT (TANPA SURAT)';
    //                 } else {
    //                     $update['status_detail'] = 'SAKIT (DENGAN SURAT)';
    //                 }
    ...
    //         */
    //         $this->db->where('id', $row->id);
    //         $this->db->update('tbl_kehadiran_harian', $update);
    //     }

    //     $rekap_absen =  $this->Presensi_model->get_absensi_pegawai($pin, $bulan, $tahun);
    //     $totalTelat = 0;
    //     $totalPawal = 0;
    //     $numIzin = 0;
    //     $izin_half = 0;
    ...
    //             case 'IZIN':
    //                 if (strpos(strtoupper($row->status_detail), 'FULL') !== false) {
    //                     $numIzin++;
    //                 } else {
    //                     $izin_half++;
    //                 }
    ...
    //                 if (strpos(strtoupper($row->status_detail), 'DENGAN SURAT') !== false) {
    //                     $sakit_dgn_surat++;
    //                 }
    //                 break;
    ...
    //             case 'DL':
    //                 if (strpos(strtoupper($row->status_detail), 'PENUH') !== false) {
    //                     $numDLP++;
    //                 } elseif (strpos(strtoupper($row->status_detail), 'AWAL') !== false) {
    //                     $numDLA++;
    //                 } elseif (strpos(strtoupper($row->status_detail), 'AKHIR') !== false) {
    //                     $numDLH++;
    //                 }
    //                 break;
    //         }
    ...
    //     $rekap = $this->db
    //         ->where('id_pegawai', $id_pegawai)
    //         ->where('periode', $periode) // kalau hanya yang aktif
    //         ->get('ts_rekap_absensi')
    //         ->row();
    ...
    {
        $this->Presensi_model->generate_rekap_bulanan(
            $pin,
            $id_pegawai,
            $bulan,
            $tahun   );
    ...
    function refresh_data()
    {
        ...
        if ($id_puskesmas != 1) {
            ...
            $pegawai = $this->Pegawai_model->getPegawaiByIDPuskesmas(
                $id_puskesmas,
                $jns_pegawai,
                $select   );
        } else {
            $pegawai = $this->Pegawai_model->getPegawaiByKlaster(
                $id_puskesmas,
                $id_klaster,
                $jns_pegawai,
                $select   );
    ...
    function insert_absen_ketidakhadiran()
    {
        ...
        $absen = $this->db
            ->get_where("tbl_kehadiran_harian", [
                "tanggal" => $tanggal,
                "pin" => $pin,
            ])
            ->row();
        ...
        if ($status != "") {
            ...
            // ================= IZIN / SAKIT =================
            if (in_array($status, ["IZIN", "SAKIT", "SAKIT DGN SURAT"])) {
                $jns_izin = $status == "IZIN" ? 1 : 0;
                $jns_sakit = 0;

                if ($status == "SAKIT") {
                    $jns_sakit = 1;
                } elseif ($status == "SAKIT DGN SURAT") {
                    $jns_sakit = 2;
                }
    ...
    function updateShiftReguler($pin, $data_shift)
    {
        ...
        for ($i = 0; $i < count($data_shift); $i++) {
            ...
            if ($libur) {
                ...
                $shiftDetail = $this->db
                    ->where("id", $data_shift[$i]->shift_id)
                    ->get("mst_shift_kerja")
                    ->row();
