										...
										<div class="card-header border-0 pt-6">
											...
											<div class="card-toolbar">
												...
												<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
													...
													<!--end::Export-->
													<form id="kt_modal_add_user_form" class="form" action="<?php echo base_url();?>admin/gaji/submit_import">
                                        
															<button type="submit" class="btn btn-success" data-kt-users-modal-action="submit">
															<i class="ki-duotone ki-check fs-2"></i>  <span class="indicator-label">Submit Import </span>
                                        ...
                                        ?>
											...
											<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
												<thead>
														...
														<th class="min-w-125px">Jumlah Baru</th>
														
														<th class="min-w-125px">Jumlah Lama</th>
														<th class="min-w-125px">THP</th>
												...
												<?php    
                                                    ...
                                                    for ($i = 0; $i < count($import) ; $i++) {
                                                            $nip = $import[$i]->nik;
                                                            $status = $import[$i]->status;
                                                            $keterangan = $import[$i]->keterangan;
                                                            $nilai_lama = $import[$i]->nilai_lama;
                                                            $nilai_baru = $import[$i]->nilai_baru;
                                                            $nama = $import[$i]->nama;
                                                       

                                                            if($status=='Berhasil'){
                                                                $status_update = ' <span class="badge py-3 px-4 fs-7 badge-light-success">'.$status.'</span>';
                                                            }else{
                                                                $status_update = '<span class="badge py-3 px-4 fs-7 badge-light-danger">'.$status.'</span>';
                                                            }
                                                           
															$getTKDPegawai = $this->Laporan_model->getRekapTKDPegawai($nip, $periode);
															

															if (!empty(($getTKDPegawai))) {
																$jumlahTKD = $getTKDPegawai[0]->thp;
															}else{
																$jumlahTKD = 0;
															}

															if($jumlahTKD > 5000000){
																if($nilai_baru < 300000){
																	$warning = '<i class="ki-duotone ki-cross fs-1 text-danger" >
																					<span class="path1"></span>
																					<span class="path2"></span>
																				</i>';
																}else{
                                                        ...
                                                        echo ' <tr>
                                                            ...
                                                            <td class="d-flex align-items-center">'.$nip.'</td>
                                                            <td>
                                                              <div class="d-flex flex-column">  '.$nama.'  </div>
                                                            </td>
                                                            <td>'.$status_update.'</td>
                                                            <td>'.$keterangan.'</td>
                                                            <td> '.rupiah($nilai_baru).' '.$warning .'</td>

                                                            <td>'.rupiah($nilai_lama).'</td>
															 <td>'.rupiah($jumlahTKD).'</td>
